...
.session .note.auto{color:var(--ink-dim)}
.session .note .nlabel{font-family:var(--sig);font-size:10.5px;letter-spacing:.13em;text-transform:uppercase;color:var(--ink-dim);opacity:.8}
...
function sessionCard(s){
  ...
  b.innerHTML =
    ...
    (s.note?'<div class="note'+(s.noteAuto?' auto':'')+'">'+
      (s.noteAuto?'<span class="nlabel">last commit</span> ':'')+esc(s.note)+'</div>':'')+
...
const pinBtn = $("pinBtn");
// the serve process binds loopback only, so loopback is the reliable sign a
// server is there to receive the POST — a copy of this page hosted anywhere
// else (the Pages demo, someone's intranet) must not show a dead button
if(/^https?:$/.test(location.protocol) &&
   /^(localhost|127\.0\.0\.1|\[::1\])$/.test(location.hostname)) pinBtn.hidden = false;
