...
function scanRepo(dir) {
  ...
  const project = { key: label, label: prettyName(label), path: tildify(dir), dir, warnings: [] };
...
function attachSessions(project, sessions) {
  ...
  const nameFor = (t) => {
    const s = sessionByTree.get(path.resolve(t.path));
...
function prettyName(s) {
  return s.replace(/[-_]+/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()).replace(/\bSr\b/, 'S&R').replace(/\bPwa\b/, 'PWA');
}
...
function scan({ root, sessionDir, defaultProject }) {
