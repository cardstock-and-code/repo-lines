...
/* Optional per-repo overrides, since git cannot always infer a stack:
   { "label": "Repo Lines Portal", "base": { "child-branch": "parent-branch" }, "hide": ["old-thing"] } */
...
function attachSessions(project, sessions) {
  ...
  for (const s of freshest.values()) {
    ...
    /* A sidebar that only says where a session is leaves you guessing what it
       is doing. Few sessions bother to set a note, so the branch's own last
       commit stands in — always current, since it is read at scan time rather
       than frozen into the session file. An explicit note always wins. */
    const branch = s.branch || t.branch;
    let note = s.note || null;
    let noteAuto = false;
    if (!note && branch) {
      const line = (project.lines || []).find((l) => l.id === branch);
      const last = line && line.commits[line.commits.length - 1];
      if (last && last.m) { note = last.m; noteAuto = true; }
    }
    mine.push({
      ...s, state, branch, note, noteAuto,
...
/* Folder name -> display label. `pretty` comes from ~/.repo-lines/config.json
   ("pretty": { "Rl": "Repo Lines" }) so personal shorthand stays a preference rather
